...
export class SecurityScanService {
   ...
   */
  static async scanFile(filePath: string, fileName: string): Promise<{
    safe: boolean;
    findings: Array<{
      rule_id: string;
      ...
      try {
        ...
        if (['.txt', '.md', '.json', '.xml', '.csv'].includes(fileExtension)) {
          ...
          const securityPatterns = [
            { pattern: /<script[\s\S]*?>[\s\S]*?<\/script>/gi, rule: 'script-injection', severity: 'high' as const },
            { pattern: /javascript:/gi, rule: 'javascript-protocol', severity: 'medium' as const },
            { pattern: /data:text\/html/gi, rule: 'data-uri-html', severity: 'medium' as const },
            { pattern: /vbscript:/gi, rule: 'vbscript-protocol', severity: 'medium' as const },
            { pattern: /on\w+\s*=/gi, rule: 'event-handler', severity: 'medium' as const },
            { pattern: /eval\s*\(/gi, rule: 'eval-usage', severity: 'high' as const },
            { pattern: /(?:password|secret|key|token)\s*[:=]\s*["\'][^"\']{8,}/gi, rule: 'potential-credential', severity: 'critical' as const }
          ];
   ...
   */
  private static async performBinaryChecks(filePath: string, fileName: string): Promise<{
    safe: boolean;
    findings: any[];
    confidence: number;
  }> {
    ...
    if (fileName.endsWith('.pdf')) {
      if (!signature.startsWith('255044462d')) { // %PDF- in hex
        findings.push({
    ...
    if (fileName.endsWith('.doc') || fileName.endsWith('.docx')) {
      ...
      if (!signature.startsWith('d0cf11e0') && !signature.startsWith('504b0304')) {
        findings.push({
    ...
    const safe = findings.filter(f => f.severity === 'high' || f.severity === 'critical').length === 0;
   ...
   */
  static async quarantineFile(filePath: string, scanResult: any): Promise<void> {
    try {
      ...
      // Create scan report
      const reportPath = path.join(quarantineDir, `${scanResult.scanId}_report.json`);
