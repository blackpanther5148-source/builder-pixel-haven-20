...
router.post('/', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (!validation.success) {
    throw new ValidationError('Invalid job application data', validation.error.errors);
  }
...
router.post('/:id/contacts', authenticateToken, asyncHandler(async (req: AuthRequest, res: Response) => {
  const applicationId = req.params.id;
  const userId = req.user!.id;
  
  const validation = contactSchema.safeParse(req.body);
  if (!validation.success) {
    throw new ValidationError('Invalid contact data', validation.error.errors);
  }
