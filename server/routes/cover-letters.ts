...
const coverLetterSchema = z.object({
  ...
  templateStyle: z
    .enum(["professional", "creative", "casual"])
    .optional()
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const { companyName, jobTitle, jobDescription, resumeId, templateStyle } =
      validation.data;
    const userId = req.user!.id;
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const letterId = req.params.id;
    const userId = req.user!.id;
    const { content, companyName, jobTitle, jobDescription, templateStyle } =
      req.body;
